...
from main_disen import Encoder, corruption
...
def elbo_decomposition(model, dataset_loader):
    N = dataset_loader.x.size(0)     # number of data samples
    K = model.z_dim                  # number of latent variables
    S = 1                            # number of latent variable samples
    nparams = model.q_dist.nparams
    batch_size = 1

    qz_params = model.encoder(dataset_loader.x, dataset_loader.edge_index).view(N, model.hidden_channels, model.q_dist.nparams)
    ...
    qz_samples = model.q_dist.sample(params=qz_params_expanded)
    qz_samples = qz_samples.transpose(0, 1).contiguous().view(K, N * S)

    print('Estimating entropies.')
    marginal_entropies, joint_entropy = estimate_entropies(qz_samples, qz_params, model.q_dist)

    if hasattr(model.q_dist, 'NLL'):
        nlogqz_condx = model.q_dist.NLL(qz_params).mean(0)
    else:
        nlogqz_condx = - model.q_dist.log_density(qz_samples,
            qz_params_expanded.transpose(0, 1).contiguous().view(K, N * S)).mean(1)

    if hasattr(model.prior_dist, 'NLL'):
        pz_params = model._get_prior_params(N * K).contiguous().view(N, K, -1)
        nlogpz = model.prior_dist.NLL(pz_params, qz_params).mean(0)
    else:
        nlogpz = - model.prior_dist.log_density(qz_samples.transpose(0, 1)).mean(0)


    nlogqz_condx, nlogpz = analytical_NLL(qz_params, model.q_dist, model.prior_dist)
    ...
    return dependence, information, dimwise_kl, analytical_cond_kl, marginal_entropies, joint_entropy


if __name__ == '__main__':
    'python elbo_decomposition.py -checkpt save/infomax_2.0.pth'
    import argparse
    ...
    lamb = args.checkpt.split('_')[-1]

    def load_model_data(checkpt_filename, dataset):
        path = osp.join(osp.dirname(osp.realpath(__file__)), '..', 'data', dataset)
        dataset = Planetoid(path, dataset)
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        loader = dataset[0].to(device)
        # print(loader, len(dataset))
        ...
        model.load_state_dict(torch.load(checkpt_filename), strict=False)
        model.eval()

        return model, loader

    torch.cuda.set_device(args.gpu)
    model, loader = load_model_data(args.checkpt, 'Cora')
    dependence, information, dimwise_kl, analytical_cond_kl, marginal_entropies, joint_entropy = \
        elbo_decomposition(model, loader)
    torch.save({
    ...
    }, os.path.join(args.save, f'elbo_decomposition_{lamb}.pth'))
